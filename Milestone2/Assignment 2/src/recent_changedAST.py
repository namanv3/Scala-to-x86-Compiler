...
def p_expr(p):
		...
		if (len(p) == 3 and p[1] == "return"):
			f.write(str(nextstat) + ": " + 'return' + p[2]['place']+"\n")
			nextstat += 1
		elif p[1] == "return":
...
def p_wh_b_m(p):
	...
	'''
	global nextstat
	f.write(str(nextstat) + ": " + "BEGIN:"+"\n")
	nextstat += 1
	f.write(str(nextstat) + ": " + "if" + p[-2]['place'] + "= 0 goto" + "AFTER"+"\n")
	nextstat += 1
	S.startScope()
...
def p_wh_s_m(p):
	...
	'''
	global nextstat
	f.write(str(nextstat) + ": " + "goto BEGIN"+"\n")
	nextstat += 1
	S.endScope()
...
def p_if_b_m(p):
	...
	'''
	global nextstat
	f.write(str(nextstat) + ": " + "if" + p[-2]['place'] + "= 0 goto" + "ELSE"+"\n")
	nextstat += 1
	S.startScope()
...
def p_if_e_m(p):
	...
	'''
	global nextstat
	f.write(str(nextstat) + ": " + "goto AFTER"+"\n")
	nextstat += 1
	f.write(str(nextstat) + ": " + "AFTER:"+"\n")
	nextstat += 1
...
def p_eq_expression(p):
		...
		if len(p)==2:
			...
			if p[1]['type'] == p[3]['type']:
				...
				p[0]['type'] = "BOOL"
				S.addtemptoST(p[0]['place'], p[0]['type'], S.getWidth(p[0]['type']))
				f.write(str(nextstat) + ": " + 'if' + p[1]['place'] + p[2] + p[3]['place'] + 'goto' + str(nextstat+3)+"\n")
				nextstat += 1
				f.write(str(nextstat) + ": " + p[0]['place'] + "= 0"+"\n")
				nextstat += 1
				f.write(str(nextstat) + ": " + "goto" + str(nextstat+2)+"\n")
				nextstat += 1
				f.write(str(nextstat) + ": " + p[0]['place'] + "= 1"+"\n")
				nextstat += 1
			else:
